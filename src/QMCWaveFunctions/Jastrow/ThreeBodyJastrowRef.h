 ...
 */
template<class FT>
class ThreeBodyJastrowRef : public WaveFunctionComponent
{
...
public:
  ...
  ThreeBodyJastrowRef(const ParticleSet& ions, ParticleSet& elecs, bool is_master = false)
      : Ions(ions)
  {
  ...
  void addFunc(int iSpecies, int eSpecies1, int eSpecies2, FT* j)
  {
    ...
    {
      APP_ABORT("ThreeBodyJastrowRef::addFunc  Jastrow function pointer is NULL");
    }
  }
  ...
  void check_complete()
  {
    ...
    if (!complete)
    {
      APP_ABORT("ThreeBodyJastrowRef::check_complete  J3 eeI is missing "
    ...
    if (!all_radii_match)
    {
      APP_ABORT("ThreeBodyJastrowRef::check_radii  J3 eeI are inconsistent for "
                "some ion species\n  see preceding messages for details");
    }
  }
                               ...
                               qmcplusplus::Vector<valT>& d2Uk)
  {
    ...
    Uj               = std::accumulate(val, val + kel_counter, Uj);
    valT gradF0_sum  = std::accumulate(gradF0, gradF0 + kel_counter, czero);
    valT gradF1_sum  = std::accumulate(gradF1, gradF1 + kel_counter, czero);
    valT hessF00_sum = std::accumulate(hessF00, hessF00 + kel_counter, czero);
    valT hessF11_sum = std::accumulate(hessF11, hessF11 + kel_counter, czero);
